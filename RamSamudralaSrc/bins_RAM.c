 ...
 *
 * October 26, 1995; Copyright (c) 1995 by Ram Samudrala.
 */
...
int get_distance_bin_one(double distance)
/* No zeros for the distance bin */
{
  ...
  fprintf(stderr, "get_distance_bin(): unknown distance, %f, encountered!\n", distance);
  return ERROR;
}

/******************************************************************/

int get_group_bin(char group_name[])
{
  switch (group_name[0])
    {
    case 'A':
...
/*  fprintf(stderr, "get_group_bin(): unknown group %s encountered!\n", group_name);*/
  return ERROR;
}
